...
def test_csv_missing_columns():
    ...
    batches = list(batch_generator_csv(path, headers, chunk_size=1))
    assert batches[0][0] == ('1', 'Alice', None)  # should handle missing column gracefully path = make_csv(headers, rows, delimiter=';')
    batches = list(batch_generator_csv(path, headers, chunk_size=1))om src.ingestors import batch_generator_csv
import pytest
