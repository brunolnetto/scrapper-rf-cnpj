...
class FileConversionService:
    ...
    def convert_audit_map(
        ...
        if use_parallel is None:
            ...
            use_parallel = not has_large_datasets
        ...
        if use_parallel:
            results = self._convert_parallel(table_work, output_dir, delimiter, progress_callback)
        else:
    ...
    def _convert_parallel(
        ...
        with ThreadPoolExecutor(max_workers=min(self.config.workers, 2)) as executor:
            try:
    ...
    def _get_memory_recommendation(self, total_gb: float, largest_file_mb: float) -> Dict[str, any]:
        ...
        if recommended_mb > 2048:
            ...
            workers = min(4, self.config.workers)
    ...
    def get_optimal_config(self, audit_map: Dict, unzip_dir: Path) -> ConversionConfig:
        ...
        if memory_rec["config_type"] == "large_dataset":
            config.memory_limit_mb = min(memory_rec["recommended_memory_mb"], 1200)
            config.cleanup_threshold_ratio = 0.7
            config.baseline_buffer_mb = 512
            config.row_group_size = 50000
            config.compression = "snappy"
            config.workers = 1
            
        elif memory_rec["config_type"] == "medium_dataset":
            config.memory_limit_mb = min(memory_rec["recommended_memory_mb"], 1500)
            config.cleanup_threshold_ratio = 0.75
            config.baseline_buffer_mb = 256
            config.row_group_size = 100000
            config.compression = "snappy"
            config.workers = 2
            
        else:  # standard
            config.memory_limit_mb = min(memory_rec["recommended_memory_mb"], 2000)
            config.cleanup_threshold_ratio = 0.8
            config.baseline_buffer_mb = 128
        ...
        self.logger.info(f"Optimal config selected: {memory_rec['config_type']} "
                        f"({config.memory_limit_mb}MB memory, {config.workers} workers)")
